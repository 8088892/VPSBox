...
# 版本: v2.7.0 (内置自研完全体 TCP 引擎 & 内核自适应防崩溃安全注入)
# =====================================================================
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[0;33m'
...
echo -e "  ${CYAN}内存   :${NC} 总计 ${RAM_MB} MB | 已用 $(free -m | awk '/^Mem:/{printf "%.0f", $3}') MB ($(free -m | awk '/^Mem:/{printf "%.0f", $3/$2*100}')%)"
...
if ! confirm_action "彻底卸载 VPSBox"; then continue; fi
