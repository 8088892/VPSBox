...
# 自动检测当前 DNS
local current_dns=$(grep '^nameserver' /etc/resolv.conf 2>/dev/null | awk '{print $2}' | tr '\n' ' ')
[ -z "$current_dns" ] && current_dns="未检测到（可能由 DHCP 或 systemd-resolved 动态管理）"
echo -e "${YELLOW}当前 DNS: ${GREEN}${current_dns}${NC}\n"
...
echo -e "  ${CYAN}IPv4   :${NC} ${SERVER_IPV4}"
echo -e "  ${CYAN}IPv6   :${NC} ${SERVER_IPV6}"
echo -e "  ${CYAN}时区   :${NC} ${CURRENT_TZ}"
echo -e "  ${CYAN}BBR    :${NC} $(get_bbr_status)"
# 网络状态汇总
local tcp_conn=$(ss -Ht state established 2>/dev/null | wc -l)
local egress_dev=$(ip route get 1.1.1.1 2>/dev/null | awk '/dev/{print $5; exit}')
local egress_ip=$(ip route get 1.1.1.1 2>/dev/null | awk '/src/{print $7; exit}')
[ -z "$egress_dev" ] && { egress_dev="未知"; egress_ip=""; }
echo -e "  ${CYAN}网络   :${NC} TCP 连接 ${tcp_conn} 条 | 出口 ${egress_dev}${egress_ip:+ → ${egress_ip}}"
echo ""
...
local ifaces=$(ip -o link show 2>/dev/null | awk -F': ' '!/lo/{gsub(/@.*/,"",$2); print $2}' | head -5)
...
local if_ip=$(ip -4 addr show "$iface" 2>/dev/null | awk '/inet /{print $2}' | head -1)
local if_speed=$(cat /sys/class/net/"$iface"/speed 2>/dev/null)
[ "$if_speed" = "-1" ] && if_speed=""
local detail=""
[ -n "$if_ip" ] && detail=" (${if_ip}"
[ -n "$if_speed" ] && detail="${detail}${if_ip:+ @}${if_speed}Mbps"
[ -n "$if_ip" ] && detail="${detail})"
[ "$rx" != "0.0" ] || [ "$tx" != "0.0" ] && echo -e "    ${GREEN}${iface}${NC}${detail}: 收 ${rx}GB | 发 ${tx}GB"
done
pause_for_enter
}
...
# UFW 安装拉入 netfilter-persistent 会加载 Oracle REJECT，停掉它并清理
systemctl stop netfilter-persistent 2>/dev/null
# 如果 live iptables 被载入了 REJECT 规则，删掉
