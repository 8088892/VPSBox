...
cat > “$TMP_FILE” <<EOF
{“inbounds”:[$NEW_INBOUND],“outbounds”:[{“protocol”:“freedom”,“tag”:“direct”}]}
EOF
fi
fi

```
# 事前校验核心逻辑：调用内核验证 JSON 合法性
# BUG FIX: xray 测试配置的正确参数是 -test -config，原来写的 run -test -conf 会报错
local TEST_PASS=0
if [ "$CORE_NAME" == "Sing-box" ]; then
    ...
    if "$X_BIN" -test -config "$TMP_FILE" >/dev/null 2>&1; then TEST_PASS=1; fi
fi
...
while true; do
    ...
    # BUG FIX: hy2 走 UDP，原来用 ss -tulpn 会同时检测 TCP 和 UDP 导致误判
    # 改为分别检测 TCP 和 UDP，只有 UDP 被占用才真正报错
    if ss -ulpn | grep -qw ":$HY2_PORT"; then
        echo -e "${RED}[错误] UDP 端口 $HY2_PORT 已被占用，请更换其他端口！${NC}"
        continue
    fi
    break
done
...
while true; do
    echo -e "\n  ${GREEN}1.${NC} Xray-core\n  ${GREEN}2.${NC} Sing-box"
    read -r -p "▶ 运行内核 [1-2, 默认 2, 0 取消]: " core_choice
    core_choice="${core_choice// /}"
    if [ "$core_choice" == "0" ]; then return; fi; [ -z "$core_choice" ] && core_choice=2
...
# BUG FIX: Xray hy2 inbound 目前存在已知 bug，提前警告用户
if [ "$core_choice" == "1" ]; then
    echo -e "\n${YELLOW}⚠️  注意：Xray-core 的 Hysteria2 入站 (inbound) 目前存在已知 bug，${NC}"
    echo -e "${YELLOW}    握手阶段服务端可能不响应，导致客户端超时。${NC}"
    echo -e "${YELLOW}    推荐选择 Sing-box 内核以获得稳定的 Hysteria2 支持。${NC}"
    read -r -p "▶ 是否仍要继续使用 Xray-core？(y/n, 默认 n): " force_xray
    force_xray="${force_xray// /}"
    if [[ ! "$force_xray" =~ ^[yY]$ ]]; then
        echo -e "${CYAN}已自动切换为 Sing-box 内核。${NC}"
        core_choice=2
    fi
fi
...
if [ "$core_choice" == "1" ]; then
    CORE_NAME="Xray"
    # BUG FIX: 补充缺失的 "listen":"0.0.0.0" 字段，否则 Xray 无法正确绑定 UDP 端口
...
if [ "$SERVICE_STATUS" == "active" ]; then
    ...
    echo -e "\n${RED}[错误] 配置文件检测失败或内核拒绝启动，部署已自动终止，未留下任何垃圾配置！${NC}"
    echo -e "${YELLOW}[提示] 如果选择了 Xray 内核，请尝试改用 Sing-box 内核重新部署。${NC}"
fi
pause_for_enter
```
