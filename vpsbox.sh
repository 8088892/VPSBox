...
get_term_width() {
    ...
    if [ "$cols" -gt 75 ];
then echo 75; else echo "$cols"; fi
}
...
system_update() {
    clear;
print_divider; echo -e "       🔄 更新系统与安装必备组件    ";
print_divider
    if ! confirm_action "更新系统与安装组件"; then pause_for_enter; return;
fi
...
system_clean() {
    clear;
print_divider; echo -e "       🧹 系统垃圾与废弃依赖清理    ";
print_divider
    if ! confirm_action "清理系统垃圾与冗余日志"; then pause_for_enter; return;
fi
...
change_root_password() {
    clear;
print_divider; echo -e "       🔑 修改系统 root 密码    ";
print_divider
    if ! confirm_action "修改 root 密码"; then pause_for_enter; return;
fi
...
setup_ssh_key() {
    clear;
print_divider; echo -e "       🛡️ 配置 SSH 密钥免密登录    ";
print_divider
...
then 
    ...
    if !
confirm_action "导入此 SSH 公钥"; then pause_for_enter; return; fi
...
change_ssh_port() {
    clear;
print_divider; echo -e "       🚪 修改 SSH 默认登录端口    ";
print_divider
...
do
        ...
        new_port="${new_port// /}"
        if [ "$new_port" == "0" ] ||
[ -z "$new_port" ]; then return; fi
        
        if !
[[ "$new_port" =~ ^[0-9]+$ ]] || [ "$new_port" -le 1024 ] || [ "$new_port" -ge 65535 ];
then
    ...
    if !
confirm_action "将 SSH 端口修改为 $new_port"; then pause_for_enter; return; fi
...
change_hostname() {
    clear;
print_divider; echo -e "       🏷️ 修改系统主机名 (Hostname)    ";
print_divider
...
[[ "$new_hostname" =~ ^[a-zA-Z0-9-]+$ ]]; then
    ...
    if !
confirm_action "将主机名修改为 $new_hostname"; then pause_for_enter; return; fi
...
set_china_timezone() {
    clear;
print_divider; echo -e "       🕒 修改系统时区为 [北京时间] (Asia/Shanghai)    ";
print_divider
    if ! confirm_action "修改系统时区为中国北京时间"; then pause_for_enter; return;
fi
...
manage_swap() {
    clear;
print_divider; echo -e "       💾 虚拟内存 (Swap) 一键管理    ";
print_divider
...
then
                ...
                if !
confirm_action "设置 ${input_size}MB 的 Swap"; then return; fi
...
chmod 600 /swapfile
                ...
                swapon /swapfile
                if !
grep -q "/swapfile" /etc/fstab; then
...
;;
            2)
                if !
confirm_action "关闭并删除现有 Swap"; then return; fi
                swapoff -a
                rm -f /swapfile
...
optimize_dns() {
    clear;
print_divider; echo -e "       🌐 系统 DNS 极速优化    ";
print_divider
    echo -e "${YELLOW}【防超时】将系统 DNS 切换为 Cloudflare 和 Google 公共节点，加快解析速度。${NC}"
    if !
confirm_action "将系统 DNS 替换为 1.1.1.1 和 8.8.8.8"; then pause_for_enter; return;
fi
...
get_bbr_status() {
    ...
    if [[ "$cc" == "bbr" ]];
then
        if uname -r | grep -qi "xanmod";
then echo -e "${PURPLE}BBRv3 (基于 Google 官方源码)${NC}"
        else echo -e "${GREEN}BBRv1 (Linux 系统原生)${NC}";
fi
...
echo -e "  ${GREEN}0.${NC} 返回主菜单"; print_divider; echo ""
        ...
        case $bbr_opt in
            1)
                if !
confirm_action "立即开启系统原生 BBRv1"; then continue; fi
...
EOF
                sysctl 
-p /etc/sysctl.d/99-bbr.conf > /dev/null 2>&1
...
confirm_action "卸载 BBRv3 (完成后将重启服务器)"; then continue; fi
                if !
dpkg -l | grep -qE "linux-image-(generic|amd64)"; then
...
apply_tuning() {
    local PROFILE=$1;
local TCP_RMEM=$2; local TCP_WMEM=$3
    clear; print_divider; echo -e "       ⚙️ TCP 自动调优注入    ";
print_divider
    if ! confirm_action "注入 TCP 底层调优参数"; then pause_for_enter; return;
fi
    
    read -r -p "▶ 是否在调优前备份当前参数？(y/n, 默认 y): " NEED_BACKUP
    NEED_BACKUP="${NEED_BACKUP// /}"
    [[ -z "$NEED_BACKUP" ||
"$NEED_BACKUP" =~ ^[yY]$ ]] && backup_config_silently
...
print_divider
 ...
 case $b_opt in
            1)
                if !
confirm_action "备份当前网络参数"; then continue; fi
                local ts=$(date +"%Y%m%d_%H%M%S")
                sysctl -a --pattern net.ipv4.tcp |
grep -E "rmem|wmem|congestion|sack" > "${BACKUP_DIR}/backup_${ts}.conf" 2>/dev/null
                echo -e "\n${GREEN}✅ TCP 参数备份成功！${NC}"
...
2)
                shopt -s nullglob;
local backups=("${BACKUP_DIR}"/backup_*.conf); shopt -u nullglob
                if [ ${#backups[@]} -eq 0 ];
...
do
                    echo -e "\n${CYAN}请选择要恢复的时间点：${NC}"
                    for i in "${!backups[@]}";
do echo -e "  ${GREEN}$((i+1)).${NC} 备份日期: $(stat -c "%y" "${backups[$i]}" | cut -d'.' -f1)";
done
                    ...
                    if [ "$res_opt" == "0" ];
then break; fi
                    if [[ "$res_opt" =~ ^[0-9]+$ ]] && [ "$res_opt" -ge 1 ] && [ "$res_opt" -le "${#backups[@]}" ];
then
                        if !
confirm_action "覆盖当前配置并还原至此备份"; then break; fi
...
do echo -e "  ${GREEN}$((i+1)).${NC} 备份日期: $(stat -c "%y" "${backups[$i]}" | cut -d'.' -f1)";
             ...
             if [ "$del_opt" == "0" ];
then break; fi
                    if [[ "$del_opt" =~ ^[0-9]+$ ]] && [ "$del_opt" -ge 1 ] && [ "$del_opt" -le "${#backups[@]}" ];
then
                        if !
confirm_action "永久删除此备份"; then break; fi
...
then
                        if !
confirm_action "永久清空所有备份"; then break; fi
...
check_media_unlock() {
    clear;
print_divider; echo -e "       📺 ip质量检测与流媒体解锁    ";
print_divider
...
do
        clear; print_divider;
echo -e "       📋 节点状态、分享与配置备份管理    ";
print_divider
        install_dependencies
        
        echo -e "${CYAN}--- 服务端底层配置状态 ---${NC}"
        if [ -f "/usr/local/etc/xray/config.json" ] && grep -q "inbounds" "/usr/local/etc/xray/config.json";
then
            jq -r '.inbounds[] |
"【Xray】 端口: \(.port) | 协议: \(.protocol) | 网络: \(if .protocol == "hysteria" then "udp" else (.streamSettings.network // "tcp") end) | 安全: \(.streamSettings.security // "none")"' /usr/local/etc/xray/config.json 2>/dev/null ||
echo -e "${YELLOW}配置文件解析失败。${NC}"
        else
            echo -e "${YELLOW}未检测到 Xray 节点配置。${NC}"
        fi
        
        if [ -f "/etc/sing-box/config.json" ] && grep -q "inbounds" "/etc/sing-box/config.json";
then
            jq -r '.inbounds[] |
"【Sing-box】 端口: \(.listen_port) | 协议: \(.type) | 网络: \(if .type == "hysteria2" then "udp" else (.transport.type // "tcp") end) | 安全: \(if .tls.reality.enabled then "reality" elif .tls.enabled then "tls" else "none" end)"' /etc/sing-box/config.json 2>/dev/null ||
echo -e "${YELLOW}配置文件解析失败。${NC}"
        else
...
then break; fi
        ...
        if [[ "$vn_opt" =~ ^[0-9]+$ ]] && [ "$vn_opt" -ge 1 ] && [ "$vn_opt" -le "${#links[@]}" ];
then
            ...
            if !
confirm_action "备份当前节点配置"; then continue; fi
            local ts=$(date +"%Y%m%d_%H%M%S")
            ...
            [ -f "$NODE_RECORD_FILE" ] && 
cp "$NODE_RECORD_FILE" "$bk_path/vpsbox_nodes.txt"
...
then
            ...
            if [ ${#n_backups[@]} -eq 0 ];
then
                echo -e "\n${RED}未找到节点备份记录。${NC}"; pause_for_enter;
continue
            fi
...
do
            ...
            if [ "$n_res_opt" == "0" ];
then continue; fi
            if [[ "$n_res_opt" =~ ^[0-9]+$ ]] && [ "$n_res_opt" -ge 1 ] && [ "$n_res_opt" -le "${#n_backups[@]}" ];
then
                if !
confirm_action "还原此备份 (当前配置将被覆盖，且服务会重启)"; then continue; fi
               ...
               echo -e "\n${GREEN}✅ 节点配置已成功还原！服务已重启。${NC}"
                ...
                echo -e "${RED}[错误] 输入无效编号！${NC}";
sleep 1
            fi
        else
            echo -e "\n${RED}[错误] 输入无效，请重新选择！${NC}";
sleep 1
        fi
    done
}
...
then
        ...
        jq -r '.inbounds[] |
"  - 端口: \(.port) | 主协议: \(.protocol)"' /usr/local/etc/xray/config.json 2>/dev/null
        nodes_found=1
    fi
    if [ -f "/etc/sing-box/config.json" ] && grep -q "inbounds" "/etc/sing-box/config.json";
then
        echo -e "\n${CYAN}【Sing-box 节点】${NC}"
        jq -r '.inbounds[] |
"  - 端口: \(.listen_port) | 主协议: \(.type)"' /etc/sing-box/config.json 2>/dev/null
        nodes_found=1
    fi
...
[[ "$del_port" =~ ^[0-9]+$ ]]; then
        ...
        local port_exists=0
        if [ -f "/usr/local/etc/xray/config.json" ] && jq -e ".inbounds[] | select(.port == $del_port)" /usr/local/etc/xray/config.json > /dev/null 2>&1;
then port_exists=1; fi
        if [ -f "/etc/sing-box/config.json" ] && jq -e ".inbounds[] | select(.listen_port == $del_port)" /etc/sing-box/config.json > /dev/null 2>&1;
then port_exists=1; fi
...
then
    ...
    if !
confirm_action "永久删除端口为 $del_port 的节点"; then pause_for_enter; return; fi
    
    if [ -f "/usr/local/etc/xray/config.json" ];
then
        if jq -e ".inbounds[] | select(.port == $del_port)" /usr/local/etc/xray/config.json > /dev/null 2>&1;
then
...
then
        if jq -e ".inbounds[] | select(.listen_port == $del_port)" /etc/sing-box/config.json > /dev/null 2>&1;
then
...
append_inbound() {
    ...
    if [ -f "$CONFIG_FILE" ] && grep -q "inbounds" "$CONFIG_FILE";
then
        echo -e "${YELLOW}[系统] 检测到已有配置，正在生成并验证测试配置...${NC}"
        if [ "$CORE_NAME" == "Sing-box" ];
then
            jq --argjson new_in "$NEW_INBOUND" --argjson port "$TARGET_PORT" 'del(.inbounds[] | select(.listen_port == $port)) |
.inbounds += [$new_in]' "$CONFIG_FILE" > "$TMP_FILE"
        else
            jq --argjson new_in "$NEW_INBOUND" --argjson port "$TARGET_PORT" 'del(.inbounds[] | select(.port == $port)) |
.inbounds += [$new_in]' "$CONFIG_FILE" > "$TMP_FILE"
        fi
    else
...
then
        ...
        if "$SB_BIN" check -c "$TMP_FILE" >/dev/null 2>&1;
then TEST_PASS=1; fi
    else
        local X_BIN=$(command -v xray || echo "/usr/local/bin/xray")
        if "$X_BIN" run -test -c "$TMP_FILE" >/dev/null 2>&1;
then TEST_PASS=1; fi
    fi
...
install_reality_node() {
    clear;
print_divider; echo -e "       🌍 部署 VLESS-Reality (直连低延迟 / 强力防封锁)    ";
print_divider
...
do
        ...
        PORT="${PORT// /}"
        if [ "$PORT" == "0" ];
then return; fi; [ -z "$PORT" ] && PORT=50000
...
do
        ...
        if [ "$core_choice" == "0" ];
then return; fi; [ -z "$core_choice" ] && core_choice=1
        if [[ "$core_choice" != "1" && "$core_choice" != "2" ]];
then continue; fi
        break
    done
...
then return; fi
    ...
    if [[ -z "$sni_choice" || "$sni_choice" == "1" ]];
then SNI_DOMAIN="gateway.icloud.com"
    elif [[ "$sni_choice" == "2" ]]; then SNI_DOMAIN="www.microsoft.com"
    else SNI_DOMAIN="$sni_choice";
fi
    
    if ! confirm_action "开始部署 Reality 节点"; then pause_for_enter; return;
fi
    install_dependencies
...
SHORT_ID=$(openssl rand -hex 8)
    ...
    LINK_IP="$SERVER_IP"
    if [[ "$IP_FORMAT" == "v6" ]];
then LINK_IP="[${SERVER_IP}]"; fi
    
    if [ "$core_choice" == "1" ];
then
        CORE_NAME="Xray"
        if !
command -v xray &> /dev/null; then bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install > /dev/null 2>&1;
fi
        hash -r; X_BIN=$(command -v xray || echo "/usr/local/bin/xray");
KEYS=$("$X_BIN" x25519)
        PRI=$(echo "$KEYS" | awk -F'[: ]+' '/Private/{print $NF}');
PUB=$(echo "$KEYS" | awk -F'[: ]+' '/Public/{print $NF}')
        ...
        if append_inbound "/usr/local/etc/xray/config.json" "$NEW_INBOUND" "$PORT" "Xray";
then
            systemctl restart xray && systemctl enable xray >/dev/null 2>&1;
SERVICE_STATUS=$(systemctl is-active xray)
        else SERVICE_STATUS="config_error";
fi
    else
        CORE_NAME="Sing-box"
        if !
command -v sing-box &> /dev/null; then bash <(curl -fsSL https://sing-box.app/install.sh) > /dev/null 2>&1;
fi
        hash -r; SB_BIN=$(command -v sing-box || echo "/usr/local/bin/sing-box");
...
PUB=$(echo "$KEYS" | awk -F'[: ]+' '/Public/{print $NF}')
        ...
        if append_inbound "/etc/sing-box/config.json" "$NEW_INBOUND" "$PORT" "Sing-box";
then
...
install_ws_tls_node() {
    clear;
print_divider; echo -e "       ☁️ 部署 VLESS-WS-TLS (套 CDN 优选 IP /拯救被墙机器)    ";
print_divider
...
do
        ...
        WS_PORT="${WS_PORT// /}"
        if [ "$WS_PORT" == "0" ];
then return; fi; [ -z "$WS_PORT" ] && WS_PORT=443
        if !
[[ "$WS_PORT" =~ ^[0-9]+$ ]] || [ "$WS_PORT" -lt 1 ] || [ "$WS_PORT" -gt 65535 ]; then continue;
fi
        if ss -tulpn | grep -qw ":$WS_PORT";
then echo -e "${RED}端口 $WS_PORT 已被占用！${NC}"; continue; fi
        break
    done
...
do
        ...
        cert_mode="${cert_mode// /}"
        if [ "$cert_mode" == "0" ];
then return; fi; [ -z "$cert_mode" ] && cert_mode=2
        if [[ "$cert_mode" != "1" && "$cert_mode" != "2" ]];
then continue; fi
...
"${force_continue// /}" =~ ^[yY]$ ]]; then continue; fi
    ...
    if !
confirm_action "开始部署 WS+TLS 节点并申请证书"; then pause_for_enter; return; fi
    install_dependencies
...
then
            ...
            if ss -tulpn | grep -q ":80 ";
then fuser -k 80/tcp > /dev/null 2>&1; fi
            /root/.acme.sh/acme.sh --issue -d "$DOMAIN" --standalone -k ec-256
            CERT_RES=$?
fi
    fi

    if [ "$CERT_RES" -ne 0 ] && [ "$CERT_RES" -ne 2 ];
then
        echo -e "\n${RED}[错误] 证书申请失败，中止。${NC}"
        pause_for_enter
        return
...
chown -R nobody:nobody "$CERT_DIR" 2>/dev/null
...
command -v xray &> /dev/null; then bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install > /dev/null 2>&1;
        ...
        if append_inbound "/usr/local/etc/xray/config.json" "$NEW_INBOUND" "$WS_PORT" "Xray";
then
            systemctl restart xray && systemctl enable xray >/dev/null 2>&1;
SERVICE_STATUS=$(systemctl is-active xray)
        else SERVICE_STATUS="config_error";
...
command -v sing-box &> /dev/null; then bash <(curl -fsSL https://sing-box.app/install.sh) > /dev/null 2>&1;
        ...
        if append_inbound "/etc/sing-box/config.json" "$NEW_INBOUND" "$WS_PORT" "Sing-box";
then
            systemctl restart sing-box && systemctl enable sing-box >/dev/null 2>&1;
SERVICE_STATUS=$(systemctl is-active sing-box)
        else SERVICE_STATUS="config_error";
...
install_hy2_node() {
    clear;
print_divider; echo -e "       ⚡ 部署 Hysteria2 (暴力 UDP 发包 / 抢占高带宽)    ";
print_divider
...
do
        ...
        HY2_PORT="${HY2_PORT// /}"
        if [ "$HY2_PORT" == "0" ];
then return; fi; [ -z "$HY2_PORT" ] && HY2_PORT=8443
        if !
[[ "$HY2_PORT" =~ ^[0-9]+$ ]] || [ "$HY2_PORT" -lt 1 ] || [ "$HY2_PORT" -gt 65535 ]; then continue;
fi
        if ss -tulpn | grep -qw ":$HY2_PORT";
then echo -e "${RED}端口 $HY2_PORT 已被占用！${NC}"; continue; fi
        break
    done
...
"${force_continue// /}" =~ ^[yY]$ ]]; then continue; fi
    ...
    if !
confirm_action "开始部署 Hysteria2 节点并申请证书"; then pause_for_enter; return; fi
    install_dependencies
...
command -v xray &> /dev/null; then bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install > /dev/null 2>&1;
        ...
        if append_inbound "/usr/local/etc/xray/config.json" "$NEW_INBOUND" "$HY2_PORT" "Xray";
then
            systemctl restart xray && systemctl enable xray >/dev/null 2>&1;
SERVICE_STATUS=$(systemctl is-active xray)
        else SERVICE_STATUS="config_error";
...
command -v sing-box &> /dev/null; then bash <(curl -fsSL https://sing-box.app/install.sh) > /dev/null 2>&1;
        ...
        if append_inbound "/etc/sing-box/config.json" "$NEW_INBOUND" "$HY2_PORT" "Sing-box";
then
            systemctl restart sing-box && systemctl enable sing-box >/dev/null 2>&1;
SERVICE_STATUS=$(systemctl is-active sing-box)
        else SERVICE_STATUS="config_error";
...
install_warp() {
    clear; print_divider; echo -e "       ☁️ Cloudflare WARP 一键解锁    ";
print_divider
    echo -e "${YELLOW}【用途】为 VPS 获取 Cloudflare 干净 IP，解锁流媒体及规避验证码。${NC}"
    if !
confirm_action "部署 Cloudflare WARP"; then pause_for_enter; return; fi
...
4)
                if !
confirm_action "开启防火墙并自动放行 SSH 端口"; then continue; fi
...
update_script() {
    clear;
print_divider; echo -e "       🔄 一键更新 VPSBox 脚本自身    ";
print_divider
    if ! confirm_action "从 GitHub 拉取最新版 VPSBox 并覆盖当前脚本"; then pause_for_enter; return;
fi

    echo -e "\n${CYAN}>>> 正在连接 GitHub 下载最新版本...${NC}"
    curl -sL "https://raw.githubusercontent.com/8088892/VPSBox/main/vpsbox.sh" -o /tmp/vpsbox_update.sh

    if [ -f /tmp/vpsbox_update.sh ] && grep -q "VPSBox" /tmp/vpsbox_update.sh;
then
...
uninstall_vpsbox() {
    print_separator
    echo -e "${YELLOW}【警告】此操作将彻底删除 VPSBox 
的快捷命令、本地备份及所有缓存日志。${NC}"
    if ! confirm_action "彻底卸载 VPSBox"; then return;
fi
...
echo -e "  ${CYAN}【基础系统管理与安全防护】${NC}"
    ...
    echo -e "  ${GREEN}15.${NC} 部署 VLESS-WS-TLS   
          ${YELLOW}(套 CDN 优选 IP / 拯救被墙机器)${NC}"
    ...
    echo -e "  
${GREEN}20.${NC} UFW 防火墙简单端口管理        ${YELLOW}(防呆管理 / 一键放行端口)${NC}"
...
8) optimize_dns ;;
        9) change_ssh_port ;;
        10) if [ "$RAM_GB" -le 1 ];
then apply_tuning "$HW_PROFILE" "4096 87380 16777216" "4096 16384 16777216"; elif [ "$RAM_GB" -le 2 ];
then apply_tuning "$HW_PROFILE" "8192 131072 67108864" "4096 16384 67108864"; else apply_tuning "高配" "8192 262144 1073741824" "4096 16384 1073741824"; fi ;;
11) manage_backup ;;
        12) manage_bbr ;;
