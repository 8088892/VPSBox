...
change_ssh_port() {
    ...
    if [ $? -ne 0 ]; then echo -e "\n${RED}[错误] SSH 服务重启失败，端口修改可能未生效。${NC}"; else echo -e "\n${GREEN}✅ SSH 端口已修改为 $new_port！${NC}"; echo -e "${RED}⚠️ 警告: 请确保您的云服务商防火墙已放行 $new_port 端口，否则下次将无法连接！${NC}"; fi
    pause_for_enter
}
change_hostname() {
...
nameserver 8.8.8.8
EOF
    if [ $? -ne 0 ]; then echo -e "${RED}[错误] 写入 /etc/resolv.conf 失败。${NC}"; else chattr +i /etc/resolv.conf >/dev/null 2>&1; echo -e "${GREEN}✅ 系统 DNS 已优化成功，并已锁定防止系统篡改！${NC}"; fi
    pause_for_enter
}
get_bbr_status() {
...
EOF
                ...
                if [ $? -eq 0 ]; then echo -e "${GREEN}✅ BBRv1 已成功开启！${NC}"; else echo -e "${RED}[错误] BBR 参数应用失败。${NC}"; fi
                sleep 2 ;;
            2)
...
apply_tuning() {
    ...
    BEGIN {
        ...
        if(i>120) {
            ...
            ms=(a<=512)?6:((a<=1024)?8:((a<=2048)?12:16)); adv=max(2,math_ceil(F*ms)); tp=2; sf=1.5; ba=2; qd_param=2.5; cs=2; lt=2; lct=1.5;
            ...
            cf1=clamp(log_curve(r,E_CONST,tp/2)*sf*(ba/2),0.5,3); lat_n=min(1,(i-120)/1880); lf=clamp(log_curve(lat_n,lct,1)*lt*cf1,1,8);
            ...
            notsent=int(min(W/2,524288)); fack=1; sw=5; dr=5; db=2; optm=262144; morph=(a<=256)?16384:32768; rmin=32768; wmin=32768; rdef=262144; wdef=262144;
            ...
            Q=math_ceil(min(2*max(100,T/65536),10000)); Z=0.5414*r+0.6836; ee=clamp(int(0.20*Q*Z),256,2048); et=clamp(int(0.40*Q*Z),2000,4000);
            ...
            rmin=8192; wmin=8192; rdef=87380; wdef=65536; nometrics=0; synr=3; mod_rcvbuf=1; gc3=8192; gc2=4096; gc1=1024;
        }
...
backup_config_silently() {
    ...
    if [ $? -eq 0 ]; then echo -e "${GREEN}✅ 参数已自动备份。${NC}"; else echo -e "${YELLOW}[警告] 自动备份异常或不支持当前系统。${NC}"; fi
}
manage_backup() {
    while true; do
        ...
        case $b_opt in
            1)
                ...
                local ts=$(date +"%Y%m%d_%H%M%S")
                sysctl -a 2>/dev/null | grep -E "net\.ipv4\.tcp_(rmem|wmem|congestion|sack)" > "${BACKUP_DIR}/backup_${ts}.conf"
                if [ $? -eq 0 ]; then echo -e "\n${GREEN}✅ TCP 参数备份成功！${NC}"; else echo -e "\n${RED}[错误] 备份执行失败。${NC}"; fi
                pause_for_enter ;;
            2)
...
view_deployed_nodes() {
    while true; do
        ...
        if [[ "$vn_opt" =~ ^[0-9]+$ ]] && [ "$vn_opt" -ge 1 ] && [ "$vn_opt" -le "${#links[@]}" ]; then
            ...
            echo -e "\n${GREEN}✅ 节点配置已成功备份至: $bk_path ${NC}"; pause_for_enter
        elif [[ "$vn_opt" =~ ^[rR]$ ]]; then
            ...
            if [[ "$n_res_opt" =~ ^[0-9]+$ ]] && [ "$n_res_opt" -ge 1 ] && [ "$n_res_opt" -le "${#n_backups[@]}" ]; then
                ...
                echo -e "\n${GREEN}✅ 节点配置已成功还原！服务已尝试重启。${NC}"; pause_for_enter
            else
...
install_reality_node() {
    clear; print_divider; echo -e "       🌍 部署 VLESS-Reality    "; print_divider
    while true; do
...
install_ws_tls_node() {
    clear; print_divider; echo -e "       ☁️ 部署 VLESS-WS-TLS    "; print_divider
    while true; do
...
install_hy2_node() {
    clear; print_divider; echo -e "       ⚡ 部署 Hysteria2    "; print_divider
    while true; do
        read -r -p "▶ 请输入域名 (输入 0 取消): " DOMAIN
        DOMAIN="${DOMAIN// /}"
        if [ "$DOMAIN" == "0" ]; then return; fi
    ...
    while true; do
        ...
        case $ufw_opt in
            ...
            2)
                ...
                port="${port// /}"
                if [[ "$port" =~ ^[0-9]+$ ]] && [ "$port" -ge 1 ] && [ "$port" -le 65535 ]; then ufw allow "$port"; echo -e "${GREEN}✅ 端口 $port 已成功添加放行规则！${NC}"; ufw reload > /dev/null 2>&1; else echo -e "${RED}[错误] 端口号输入无效！${NC}"; fi
                pause_for_enter ;;
            3)
                echo -e "\n${CYAN}>>> 当前规则列表：${NC}"; ufw status numbered; echo ""
                read -r -p "▶ 请输入要删除的【规则编号】: " rule_num
                rule_num="${rule_num// /}"
                if [[ "$rule_num" =~ ^[0-9]+$ ]]; then ufw --force delete "$rule_num" || echo -e "${RED}[错误] 删除规则失败。${NC}"; echo -e "${GREEN}✅ 规则 $rule_num 已尝试删除！${NC}"; fi
                pause_for_enter ;;
            4)
                ...
                echo -e "\n${GREEN}✅ 防火墙已成功开启！当前 SSH 端口 $CURRENT_SSH_PORT 已安全放行。${NC}"; pause_for_enter ;;
            5)
                if ! confirm_action "彻底关闭防火墙"; then continue; fi
                ufw disable || { echo -e "${RED}[错误] 关闭防火墙失败。${NC}"; pause_for_enter; continue; }
                echo -e "${GREEN}✅ 防火墙已完全关闭！${NC}"; pause_for_enter ;;
            0) break ;;
            *) echo -e "\n${RED}输入无效！${NC}"; sleep 1 ;;
        esac
    done
...
update_script() {
    ...
    if [ -f /tmp/vpsbox_update.sh ] && grep -q "VPSBox" /tmp/vpsbox_update.sh; then
        ...
        echo -e "\n${GREEN}✅ VPSBox 已成功更新到最新版本！即将自动重启脚本...${NC}"; sleep 2; exec "$SHORTCUT_PATH"
    else
...
while true; do
    ...
    echo -e "${PURPLE}           VPS Box 节点部署与服务器管家 v2.6.0 ${NC}"
    print_divider
